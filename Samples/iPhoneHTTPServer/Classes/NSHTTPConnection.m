...
@implementation NSHTTPConnection
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    } else if ([relativePath isEqualToString:@"/test"]) {
        NSData *response = [@"<html><body>{\"errorCode\": \"0000\"}</body></html>" dataUsingEncoding:NSUTF8StringEncoding];
        return [[HTTPDataResponse alloc] initWithData:response];
    } else {
        NSObject<HTTPResponse> *response = [[NSHttpRouter defaultRouter] route:relativePath params:[self parseGetParams]];
        return response;
    }
